...
while(1):
    if GPIO.input(knop1_pin)== 0:
        if Button_state1==0:
            print("parkeerplek 1 op verdieping 1 is bezet")
            #payload= {"id": "Parkeerplaats 1", "verdieping": "Verdieping 1", "status": "Bezet"}
            GPIO.output(led1rood_pin, True)
            GPIO.output(led1groen_pin, False)
            Button_state1=True
            sleep(.5)
        else:
            print("parkeerplek 1 op verdieping 1 is vrij")
            GPIO.output(led1groen_pin, True)
            GPIO.output(led1rood_pin, False)
            #payload= {"id": "Parkeerplaats 1", "verdieping": "Verdieping 1", "status": "Vrij"}
            Button_state1=False
            sleep(.5)
    if GPIO.input(knop2_pin) == 0:
        if Button_state2 == 0:
            print("parkeerplek 2 op verdieping 1 is bezet")
            #payload = {"id": "Parkeerplaats 2", "verdieping": "Verdieping 1", "status": "Bezet"}
            GPIO.output(led2groen_pin, False)
            GPIO.output(led2rood_pin, True)
            Button_state2 = True
            sleep(.5)
        else:
            print("parkeerplek 2 op verdieping 1 is vrij")
            GPIO.output(led2groen_pin, True)
            GPIO.output(led2rood_pin, False)
            #payload = {"id": "Parkeerplaats 2", "verdieping": "Verdieping 1", "status": "vrij"}
            Button_state2 = False
            sleep(.5)
    if GPIO.input(knop3_pin) == 0:
        if Button_state3 == 0:
            print("parkeerplek 3 op verdieping 1 is bezet")
            #payload = {"id": "Parkeerplaats 3", "verdieping": "Verdieping 1", "status": "Bezet"}
            GPIO.output(led3groen_pin, False)
            GPIO.output(led3rood_pin, True)
            Button_state3 = True
            sleep(.5)
        else:
            print("parkeerplek 3 op verdieping 1 is vrij")
            GPIO.output(led3groen_pin, True)
            GPIO.output(led3rood_pin, False)
            #payload = {"id": "Parkeerplaats 3", "verdieping": "Verdieping 1", "status": "vrij"}
            Button_state3 = False
            sleep(.5)
